...
import AdUnit from "../../../../Components/Adsense/AdUnit.tsx";
...
const LiveTab: FC<IWithRouter> = (props) => {
  ...
  const { mutate, isPending, reset } = useMutationQuery(
    {
      mutationKey: [endpoints.CreateLive, "POST"],
      onSuccess: (data: any) =>
        navigate(
          paths.LiveNow.replaceWithObject({
  ...
  const renderLiveScreen = () => {
    ...
    return (
      <form
        ...
        className="tablet:px-[10%] laptop:px-[20%] py-[5%]"
      >
        <MultiSelectImage
          showOnlyPhoto={false}
  ...
  return (
    <>
      <Card>{renderLiveScreen()}</Card>
      <div className="my-4 justify-items-center">
        <AdUnit slot="3887464372" />
      </div>
    </>
  );
};
