...
const AccountDeletePage: FC<IWithRouter> = (props) => {
  ...
  const { mutate, isPending } = useMutationQuery(
    {
      mutationKey: [Endpoints.DeleteAccount, "POST"],
  ...
  const onSubmit = (data: any) => {
    // if(selector.isLoginEmail != watch("email") ||selector.isLoginPhone != watch("email")  ) return toast.error("Unauthorised email/phone")
    // else
    if (watch("reason.value") === 5) {
        ...
        >
          <div className="my-[2%] flex flex-col gap-4 w-[80%] laptop:w-[60%] mx-auto">
            {/* <TextField
              ...
              name="email"
              type="email"
              placeHolder="Confirm your email or phone"
