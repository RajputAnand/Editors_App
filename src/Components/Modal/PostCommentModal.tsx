...
const PostCommentModal:FC<IPostCommentModal> = (props) => {
    const {open, handle, post_id, onClose, usePublic} = props
    const {me} = useUser();
    ...
    const {fetchNextPage, isLoading, hasNextPage, data, refetch, isRefetching} = useInfiniteQuery({
    ...
    const onSubmit = (data: any) =>{
        ...
        if(data.text === undefined) return toast.error("Please enter your comment")
       return mutate({
            ...data,
            title: "Comment",
            location: "",
    ...
    const renderHeader = () => {
        ...
        return (
            <div className="flex items-center justify-between mb-[1.25rem]">
                <Typography variant={"title"} size={"medium"} className="!text-[1.25rem] !font-medium !leading-[1.89669rem] text-secondary-light">Remarks</Typography>
    ...
    const renderListing = () => {
        ...
        if (dataLength === 0) return (
            <div className="min-h-[15rem] flex items-center justify-center">
                <Typography variant="headline" size="small" className="text-secondary-light/40">No remarks yet</Typography>
            </div>
        )
        return (
            <InfiniteScroll
            ...
            >
                {
                    data?.pages?.map((page) => {
                        return page?.data?.slice(0,!seeMore ? 2 : LIMIT_PAGINATION).map?.((item: any, key: number) => (
                            <CommentCard usePublic={usePublic} refetch={refetch} {...item} key={key}/>
                        ))
                    })
                }
                { !seeMore && dataLength > 2 && <MoreComment onClick={handleClickSeeMore}/> }
            </InfiniteScroll>
        )
    }
    ...
    const renderCommentInput = () => {
        ...
        const renderSubmitButton = () => {
            if (isPending) return <Loading className="px-3" iconClassName="w-[2rem] h-[2rem]"/>
            return (
                <MaterialSymbol onClick={handleSubmit(onSubmit)} icon={"send"} className="text-surface-20 !text-[2rem] px-3 cursor-pointer" as={"div"} />
            )
        }
        ...
        <Transition appear show={open} as={Fragment}>
            <Dialog as="div" className="relative z-10" onClose={handleOnClose}>
                <div className="fixed inset-0 overflow-y-auto">
                    <div className="flex min-h-full mt-16 items-end justify-center p-4 text-center tablet:items-center tablet:justify-end tablet:pr-[2.88rem] laptop:justify-center laptop:p-0">
                        <Transition.Child
                            as={Fragment}
