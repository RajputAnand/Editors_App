...
const PromptModal:FC<IPromptModal> = (props) => {

    const {isOpen, closeModal, children, onClickYes, hideButtons = false, bgClassName} = props
                                    ...
                                    !hideButtons && (
                                        <div className="flex items-center justify-center gap-2">
                                            <Button variant="outline"
                                                    ...
                                                    onClick={closeModal}>
                                                <MaterialSymbol icon={"close"} fill
                                                                className="!text-primary !text-[1.5rem] !leading-none"/>
                                                Cancel
                                            </Button>
                                            <Button variant="primary"
                                                    className="!px-4 py-[0.6rem] flex items-center !bg-red-1 justify-center !leading-none !border-outline !min-w-[7rem]"
                                                    onClick={onClickYes}>
                                                <MaterialSymbol icon={"logout"} fill
                                                                className="!text-white !text-[1.5rem] !leading-none"/>
                                                Yes
                                            </Button>
                                        </div>
                                    )
