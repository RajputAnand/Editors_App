...
import MultiSelectImage from "../../Inputs/ImageSelect/MultiSelectImage";
import TextField from "../../Inputs/TextField";
import TextAreaField from "../../Inputs/TextAreaField";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { validationSchema } from "../../../Pages/Dashboard/AddPost/AddPostPage";
import { useMutationQuery } from "../../../Api/QueryHooks/useMutationQuery";
import toast from "react-hot-toast";
import { Endpoints } from "../../../Api/Endpoints";
import { queryClient } from "../../../Api/Client";
import NetworkMemberPostMension from "../../Inputs/PostMension/NetworkPostMension";
...
export const AddNetworkPostModal: FC<IAddNetworkPostModal> = (props) => {
  const { handle, open, onClose, networkGroupId } = props;
  ...
  const { mutate, isPending, progress } = useMutationQuery(
    {
      mutationKey: [Endpoints.AddNetworkPost, "POST"],
      onSuccess: handlePostOnSuccess,
      onError: () => {
        toast.error(
          "Something went wrong or tagged user is not in network group"
        );
      },
    },
    true,
      ...
      <div className="w-full min-w-[2.75rem] rounded-lg  bg-white p-[0.06rem] sm:p-[0.06rem]">
        <div className="flex items-start justify-between">
          <div className="flex flex-col items-start">
            <h2 className="text-[#0E0E0E] font-bold text-lg">Add Post</h2>
          </div>
        </div>
