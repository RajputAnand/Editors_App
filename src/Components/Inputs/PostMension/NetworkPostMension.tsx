...
const NetworkMemberPostMension: FC<IMensionPost> = (props) => {
  ...
  const removeMe = [selector.isLoginUserId];
  ...
  const { hasNextPage, fetchNextPage, isLoading, data } = useInfiniteQuery({
    queryKey: [
      ...
      "GET",
      {
        ...
        remove_member_user_ids: JSON.stringify(removeMe) || "",
  ...
  const users = data?.pages[0]?.data?.map((user: any) => ({
    display: user.member_user.name,
    id: user.member_user_id,
    avatar: user.member_user.profile_image,
  }));
...
export default NetworkMemberPostMension;
