...
const LiveInviteAdmiring: FC<ILiveInviteUsers> = ({ config, liveId }) => {
  const [searchQuery, setSearchQuery] = useState<string>("");

  const { isLoading, data } = useGetQuery({ queryKey: [(Endpoints.ListAdmirning).replaceWithObject({ id: config?.uid?.toString() }), "GET", { page: 1, per_page: 999999 }], refetchOnMount: true })
...
export default LiveInviteAdmiring